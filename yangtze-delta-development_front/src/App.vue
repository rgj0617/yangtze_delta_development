<script lang="ts">
import { ref, onMounted } from "vue";
export default {
  setup() {
    const message = ref("Hello, Vue 3!");

    onMounted(() => {
      console.log(
        "%cOpenGMS",
        " text-shadow: 0 1px 0 #ccc,0 2px 0 #c9c9c9,0 3px 0 #bbb,0 4px 0 #b9b9b9,0 5px 0 #aaa,0 6px 1px rgba(0,0,0,.1),0 0 5px rgba(0,0,0,.1),0 1px 3px rgba(0,0,0,.3),0 3px 5px rgba(0,0,0,.2),0 5px 10px rgba(0,0,0,.25),0 10px 10px rgba(0,0,0,.2),0 20px 20px rgba(0,0,0,.15);font-size:5em"
      );
      console.log(
        "Develop: Guangjin Ren , Zheng Hu , Chenchao Zhong\nGuide: Professor Yang Li , Professor Min Chen , Manager Yu Han , Development Engineer Jian Xu\nThe ownership of this website belongs to Nanjing Normal University\n"
      );
    });

    return {
      message,
    };
  },
};
</script>

<template>
  <div id="app">
    <router-view />
  </div>
</template>

<style scoped>
.logo {
  height: 6em;
  padding: 1.5em;
  will-change: filter;
  transition: filter 300ms;
}
.logo:hover {
  filter: drop-shadow(0 0 2em #646cffaa);
}
.logo.vue:hover {
  filter: drop-shadow(0 0 2em #42b883aa);
}
</style>
