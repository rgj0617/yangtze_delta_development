...
function selectFeature(e) {
  ...
  if(features.length > 0) {
    ...
    const selectedCity = clickedFeature.properties.name;
    selectedCityStore.set(selectedCity)
    // 更新高亮图层的过滤器，只显示选中的要素
    map.setFilter('highlightLayer', ['==', ['get', 'name'], selectedCity]);
...
function bindMapInteractions(value) {
  map.off("mousemove", "geojsonLayer", showPointer);
  map.on("mousemove", "geojsonLayer", (e) => showPointer(e, value));

  map.off("mouseout", "geojsonLayer", removePointer);
  map.on("mouseout", "geojsonLayer", removePointer);

  map.off("click", "geojsonLayer", selectFeature);
  map.on("click", "geojsonLayer", (e) => selectFeature(e));
}
