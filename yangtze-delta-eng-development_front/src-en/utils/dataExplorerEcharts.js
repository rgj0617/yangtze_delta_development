...
import detailUnformatted from "@/assets/json/scoreDetail-en.json";
import rankingUnformatted from "@/assets/json/scoreRanking.json";
import Indicators from "@/assets/json/secondaryIndicators.json";
import { scoreFormatEn } from "@/utils/format.ts";

const ranking = scoreFormatEn(rankingUnformatted);
const detail = scoreFormatEn(detailUnformatted);
...
export const getAnnualScore = (dataset, value) => {
  ...
  let option = {
    title: {
      ...
      subtext:
        "tips：Scroll the mouse wheel or drag the scroll bar on the right to view more",
    },
    xAxis: {
      type: "value", // 设置为类目轴
      position: "top",
...
function handlePieData(dataset) {
  ...
  let value = [];
  let name = "";
  let barName = ["Innovation", "Coordination", "Green", "Openness", "Sharing"];
